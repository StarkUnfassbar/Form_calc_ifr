...
function periodError(formBlock){
    ...
    if(blockInputsLength > 1){
                    ...
                    if(window.innerWidth <= 600){
                        if(selects[0].value === ""){
                            ...
                            if(window.innerWidth <= 380){
                                blockInput.lastElementChild.style.bottom = "140px";
                            } else{
                                blockInput.lastElementChild.style.bottom = "130px";
                            }

                            selects[0].closest(".block_select").style.paddingBottom = "40px";
                        } else{
                            blockInput.lastElementChild.classList.add("first");
        ...
        if(periodChronologyInside(formBlock, blockInputsLast)){
            isChronologyInside = true;
            datasetCount += 1;
        }

        if(periodIntersectionInside(formBlock, blockInputsLast)){
            datasetCount += 1;

            if(datasetCount > 0){
                formBlock.dataset.clear = "false";
            } else{
                formBlock.dataset.clear = "true";
            }
    ...
    if(!isChronologyInside && periodCheckChronology(formBlock, true)){
        blockInputs.forEach(blockInput => {
            blockInput.classList.add("_error");
...
function periodChronologyInside(formBlock, blockInputsLast){
    let errorHtml = `
        <p class="error_text">Периоды заполняются по хронологии: от самого раннего до самого позднего</p>
    `;
    ...
    formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs1 => {
        let selectsHead1 = blockInputs1.querySelectorAll(".select .select__head");

        let date1 = new Date(selectsHead1[0].value.split('.').reverse().join('-'));
        let date2 = new Date(selectsHead1[1].value.split('.').reverse().join('-'));

        if(date2 < date1){
            if(!blockInputs1.querySelector(".error_text.first")){
                blockInputs1.classList.add("_error");
                blockInputs1.insertAdjacentHTML("afterbegin", errorHtml);
                blockInputs1.querySelector(".error_text").classList.add("first");

                if(blockInputs1.querySelector(".error_text.first") && blockInputs1.querySelector(".error_text.second")){
                    blockInputs1.querySelector(".error_text.first").style.width = "50%";
                    blockInputs1.style.paddingBottom = "50px";
                }
...
function periodIntersectionInside(formBlock, blockInputsLast){
    let errorHtml = `
    ...
    let allBlockInputs = Array.from(formBlock.querySelectorAll(".block_inputs:not(.radio)"));

    formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs1 => {
        if(blockInputs1 != blockInputsLast){
            formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs2 => {
                if(blockInputs2 != blockInputs1 && allBlockInputs.indexOf(blockInputs1) < allBlockInputs.indexOf(blockInputs2)){
                    let selectHead1 = blockInputs1.querySelectorAll(".select .select__head");
                    let selectHead2 = blockInputs2.querySelectorAll(".select .select__head");
    
                    let startDate1 = selectHead1[0].value
                    startDate1 = new Date(startDate1.split('.').reverse().join('-'));
                
                    let startDate2 = selectHead2[0].value
                    startDate2 = new Date(startDate2.split('.').reverse().join('-'));
                
                    let endDate1 = selectHead1[1].value;
                    endDate1 = new Date(endDate1.split('.').reverse().join('-'));
                
                    let endDate2 = selectHead2[1].value;
                    endDate2 = new Date(endDate2.split('.').reverse().join('-'));
                
                    if(
                        (startDate1 <= endDate2 && startDate1 >= startDate2) || (startDate2 <= endDate1 && startDate2 >= startDate1) ||
                        (isNaN(endDate2.getTime()) && startDate1 >= startDate2)
                    ){
                        if(
                            !blockInputs1.querySelector(".error_text.first") ||
                            (blockInputs1.querySelector(".error_text.first") && blockInputs1.querySelector(".error_text.first").innerText == "Отсутствует одна из дат")
                        ){
                            if(blockInputs1.querySelector(".error_text.first")){
                                blockInputs1.removeChild(blockInputs1.querySelector(".error_text.first"))
                            }
                        ...
                        if(window.innerWidth <= 720){
                            if(isNaN(endDate2.getTime())){
                                blockInputs2.classList.add("_error");
                                blockInputs2.insertAdjacentHTML("afterbegin", errorHtml);
                                blockInputs2.querySelector(".error_text").classList.add("first");
                                blockInputs2.querySelector(".error_text.first").style.bottom = "130px";
                                blockInputs2.style.gap = "70px";
                            }
                        }
    
                        if(
                            !blockInputs2.querySelector(".error_text.first") ||
                            (blockInputs2.querySelector(".error_text.first") && blockInputs2.querySelector(".error_text.first").innerText == "Отсутствует одна из дат")
                        ){
                            if(blockInputs2.querySelector(".error_text.first")){
                                blockInputs2.removeChild(blockInputs2.querySelector(".error_text.first"))
                            }
                            ...
                            if(blockInputs2.querySelector(".error_text.first") && blockInputs2.querySelector(".error_text.second")){
                                blockInputs2.querySelector(".error_text.first").style.width = "50%";
                                blockInputs2.style.paddingBottom = "50px";
                            } else{
                                blockInputs2.style.paddingBottom = "";
                            }
                        }
        ...
        if(period.dataset.clear == "true"){
            period.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs => {
                if(blockInputs.querySelector(".error_text") && blockInputs.querySelector(".error_text").innerText === "Периоды не могут совпадать и/или пересекаться"){
                    ...
                    blockInputs.classList.remove("_error");
                    blockInputs.style.paddingBottom = "";
                }
            });
