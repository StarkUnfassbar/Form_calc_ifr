...
function periodIntersectionStart(formBlock){
    ...
    if(formBlock.id === "period-2"){
        ...
        if(
            (!period2.querySelector(".error_text") || period2.querySelector(".error_text").innerText === "Периоды не могут совпадать и/или пересекаться" || (!periodCheckChronology(period2, false) && period4.querySelector(".error_text").innerText === "Периоды заполняются по хронологии: от самого раннего до самого позднего")) && 
