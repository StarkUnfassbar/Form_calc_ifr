...
function periodError(formBlock){
    ...
    if(blockInputsLength > 1){
        ...
        if(periodChronologyInside(formBlock, blockInputsLast) || periodIntersectionInside(formBlock, blockInputsLast)){
            return;
        }
...
function periodChronologyInside(formBlock, blockInputsLast){
    ...
    formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs1 => {
        if(blockInputs1 != blockInputsLast){
            ...
            let index1 = allBlockInputs.indexOf(blockInputs1);
    
            for (let i = index1 + 1; i < allBlockInputs.length; i++) {
                let blockInputs2 = allBlockInputs[i];
                if(blockInputs2 != blockInputs1){
                    let selectsHead2 = blockInputs2.querySelectorAll(".select .select__head");

                    let date3 = new Date(selectsHead1[1].value.split('.').reverse().join('-'));
                    let date4 = new Date(selectsHead2[0].value.split('.').reverse().join('-'));

                    if (date4 <= date3){
                        if(
                            blockInputs1.querySelector(".error_text.first") &&
                            blockInputs1.querySelector(".error_text.first").innerText === "Поле должно быть заполнено"
                        ){
                            blockInputs1.classList.add("_error");
                            blockInputs1.insertAdjacentHTML("afterbegin", errorHtml);
                            blockInputs1.querySelector(".error_text").classList.add("second");
        
                            blockInputs1.querySelector(".error_text.second").style.width = "50%";
                            blockInputs1.style.paddingBottom = "50px";
    ...
    formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs1 => {
        if(blockInputs1 != blockInputsLast){
            formBlock.querySelectorAll(".block_inputs:not(.radio)").forEach(blockInputs2 => {
                if(blockInputs2 != blockInputs1 && blockInputs2 != blockInputsLast){
